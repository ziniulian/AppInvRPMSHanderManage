...
public class StockInActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edtStorageLocation = (TextView) findViewById(R.id.edtStorageLocation);

		ListView mEpcListView = (ListView) this.findViewById(R.id.lstPartsStockInView);
	...
	protected void onDestroy() {
		if (listPartsEntity.size() > 0) {
			...
			String remark = "";
			String info = storageLocationStr + "," + user + "," + opTime + "," + remark;

//			SqliteHelper.SaveOpRecord(ps, OpType.StockIn, info, storageLocationStr, user);
			SqliteHelper.SaveOpRecord(ps, OpType.StockIn, info);	// 保存操作记录
	...
	private OnClickListener btnConfigClickListener = new OnClickListener() {
		public void onClick(View v) {
			AlertDialog.Builder builder = new Builder(StockInActivity.this, R.style.AppTheme);
	...
	// 判断库位是否能存放
	private boolean canStockIn(String storageLocationStr, PartsEntity partsEntiry) {

		StorageLocationEntity entity = SqliteHelper.queryStorageLocationByCode(storageLocationStr);
		if (entity != null) {
			...
			if (entity.PartsAllow.length() == 10) {
				...
				if (!t5.equals("00")) {
					if (!partsEntiry.PartsCode.subSequence(0, 2).equals(t5)) {
						showToast(String.format("%s不允许放此库位", partsEntiry.PartsName));
						return false;
					}
				...
				if (!factotyCode.equals("00")) {
					if (!partsEntiry.PartsCode.subSequence(2, 4).equals(factotyCode)) {
						showToast(String.format("%s不允许放此库位", partsEntiry.PartsName));
						return false;
					}
				...
				if (!partsSort.equals("0")) {
					if (!partsEntiry.PartsCode.subSequence(4, 5).equals(partsSort)) {
						showToast(String.format("%s不允许放此库位", partsEntiry.PartsName));
						return false;
					}
				...
				if (!partsType.equals("00")) {
					if (!partsEntiry.PartsCode.subSequence(5, 7).equals(partsType)) {
						showToast(String.format("%s不允许放此库位", partsEntiry.PartsName));
						return false;
					}
				...
				if (!partsName.equals("000")) {
					if (!partsEntiry.PartsCode.subSequence(7, 10).equals(partsName)) {
						showToast(String.format("%s不允许放此库位", partsEntiry.PartsName));
						return false;
					}
				}
			...
			if (entity.MaxVolume > 0) {
				int pairsNum = SqliteHelper.queryPairsNumByStorageLocationCode(storageLocationStr);
			...
			try {
				if (msg instanceof RXD_TagData) {
					...
					if (storageLocationStr.length() > 0) {
						// 找到配件信息并且验证是否允许入库
						if (UtilityHelper.CheckEpc(epc) == 0 && UtilityHelper.CheckUserData(userData, OpType.StockIn)) {
							if (IsValidEpc(epc, false)) {
								...
								if (partsCode.length() > 0) {
									...
									if (!isExsit) {
										...
										cupcd = partsCode;
										cardOperationHandler.sendMessage(cardOperationHandler.obtainMessage(DATA_ARRIVED_PAIRS, 0, 0, new String[] {tid, epc, "0200", partsCode, userData.substring(0, 4)}));
									}
								}
							}
						}
						...
						// 找到库位信息
						if (IsValidEpc(epc, true) && storageLocationStr.length() == 0) {
	...
	private Handler cardOperationHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case DATA_ARRIVED_STORAGE_LOCATION:// 接收库位数据
				...
				txtRemark.setText("温馨提示：" + UtilityHelper.getStorageLocationInfo(storageLocationStr));
				txtInfo.setText("已入库数：0");
	...
	public class PartsAdapter extends SimpleAdapter {
		...
		PartsAdapter(Context context, List<Map<String, Object>> data, int resource, String[] from, int[] to) {
