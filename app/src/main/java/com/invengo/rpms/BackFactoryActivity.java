...
public class BackFactoryActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_backfactory);

		reader.onMessageNotificationReceived.clear();
		reader.onMessageNotificationReceived.add(BackFactoryActivity.this);
		...
		mEpcListView = (ListView) this.findViewById(R.id.lstPartsBackFactoryView);
...
//						+ "',Code='" + stationCodeStr
			...
			SqliteHelper.SaveOpRecord(ps, OpType.BackFactory, info);	// 保存操作记录
	...
	private OnClickListener btnConfigClickListener = new OnClickListener() {
		public void onClick(View v) {
			AlertDialog.Builder builder = new Builder(BackFactoryActivity.this, R.style.AppTheme);
			builder.setTitle("温馨提示");
			builder.setMessage(getResources().getString(
					R.string.pairsBackFactoryTipInfo));
			...
			try {
				if (msg instanceof RXD_TagData) {
					...
					if (UtilityHelper.CheckEpc(epc) == 0
							&& UtilityHelper.CheckUserData(userData, OpType.BackFactory)) {
						if (IsValidEpc(epc, false)) {
							...
							if (partsCode.length() > 0) {
								...
								if (!isExsit) {
									...
									cupcd = partsCode;
									cardOperationHandler.sendMessage(cardOperationHandler.obtainMessage(DATA_ARRIVED_PAIRS, 0, 0, new String[] {tid, epc, "0C00", partsCode, userData.substring(0, 4)}));
								}
							}
						}
					}
